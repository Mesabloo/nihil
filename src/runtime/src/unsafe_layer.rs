...
pub fn coerce_to_vexpr<'a>(ex: *const VExpr_s) -> VExpr<'a> {
    ...
    match ex.ctor {
        ...
        VExpr_s_VExpr_Cons_CrECharacter => VExpr::ECharacter(unsafe { ex.value.eCharacter.v_c } as u8 as char),
        VExpr_s_VExpr_Cons_CrEId => {
            let name = unsafe { CStr::from_ptr(ex.value.eId.v_name) }.to_str().expect("Cannot decode UTF8 string");
            VExpr::EId(name)
        },
        ...
        VExpr_s_VExpr_Cons_CrETuple => {
            let exprs = vec_from_ptr(unsafe { ex.value.eTuple.v_vals }, unsafe { ex.value.eTuple.n } as usize)
                .into_iter()
                .map(coerce_to_vexpr)
                .collect();
            VExpr::ETuple(exprs)
        ...
        VExpr_s_VExpr_Cons_CrEMatch => {
            ...
            let exprs = vec_from_ptr(unsafe { ex.value.eMatch.v_branches }, unsafe { ex.value.eMatch.n } as usize)
                .into_iter()
        ...
        VExpr_s_VExpr_Cons_CrELet => {
            let decls = vec_from_ptr(unsafe { ex.value.eLet.v_decls }, unsafe { ex.value.eLet.n } as usize)
                .into_iter()
                .map(coerce_to_vdecl)
                .collect();
...
fn coerce_to_vpattern<'a>(pat: *const VPattern_s) -> VPattern<'a> {
    ...
    match pat.ctor {
        VPattern_s_VPattern_Cons_CrPInteger => VPattern::PInteger(unsafe { pat.value.pInteger.p_i } as i64),
        VPattern_s_VPattern_Cons_CrPDouble => VPattern::PDouble(unsafe { pat.value.pDouble.p_d }),
        VPattern_s_VPattern_Cons_CrPCharacter => VPattern::PCharacter(unsafe { pat.value.pCharacter.p_c } as u8 as char),
        VPattern_s_VPattern_Cons_CrPId => {
            let name = unsafe { CStr::from_ptr(pat.value.pId.p_name) }.to_str().expect("Cannot decode UTF8 string");
            VPattern::PId(name)
        },
        VPattern_s_VPattern_Cons_CrPTuple => {
            let pats = vec_from_ptr(unsafe { pat.value.pTuple.p_patterns }, unsafe { pat.value.pTuple.n } as usize)
                .into_iter()
        ...
        VPattern_s_VPattern_Cons_CrPConstructor => {
            let name = unsafe { CStr::from_ptr(pat.value.pConstructor.p_name) }.to_str().expect("Cannot decode UTF8 string");
            let args = vec_from_ptr(unsafe { pat.value.pConstructor.p_args }, unsafe { pat.value.pConstructor.n } as usize)
                .into_iter()
                .map(coerce_to_vpattern)
                .collect();
...
fn coerce_to_vdecl<'a>(ptr: *const VDecl_s) -> (&'a str, VExpr<'a>) {
    ...
    (name.to_str().expect("Cannot decode UTF8 string"), coerce_to_vexpr(ex))
}
