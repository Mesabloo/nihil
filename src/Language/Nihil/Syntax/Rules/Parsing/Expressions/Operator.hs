...
module Language.Nihil.Syntax.Rules.Parsing.Expressions.Operator where
...
operator :: Parser (Located Atom)
operator = do
    (span', op) <- getPositionInSource $
        ticks identifier <|> opSymbol
    pure (AOperator op :@ Just span')